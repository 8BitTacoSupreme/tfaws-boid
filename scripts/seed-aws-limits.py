#!/usr/bin/env python3
"""Seed canon/aws-limits.json with AWS service quotas that trip up Terraform users.

Focus: limits that cause silent failures or confusing errors, not the full quota catalog.
15 services matching LocalStack compose: S3, EC2, IAM, RDS, ECS, ELBv2, Lambda,
DynamoDB, SQS, SNS, Route53, ACM, Secrets Manager, CloudWatch, STS
"""
...
FILENAME = "aws-limits.json"


def build_limits() -> list[dict]:
    """Build the AWS service limits entries."""
    return [
        # ── EC2 ───────────────────────────────────────────────────────
        {
            "service": "ec2",
...
def main() -> None:
    ...
    data["limits"] = build_limits()

    path = save_canon(FILENAME, data)
    print(f"Wrote {len(data['limits'])} limit entries")
